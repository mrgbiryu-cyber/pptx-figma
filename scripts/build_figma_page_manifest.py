...
def is_fullpage_overlay_candidate(node, page_bounds):
    ...
    if not large or node.get("children"):
        return False
    for fill in fills:
...
def main():
    parser = argparse.ArgumentParser(description="Build manifests from exported Figma page JSON files.")
    parser.add_argument("--input", nargs="+", required=True, help="Input Figma page JSON paths")
    ...
    for input_name in args.input:
        page_path = Path(input_name).resolve()
        manifest = build_manifest(page_path)
        output_path = output_dir / f"{page_path.stem}.manifest.json"
